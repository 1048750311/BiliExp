...
const utils = {
    ...
    * infiniteNumber() {
    ...
    log: {
        ...
        _iso_time: () => {
            const d = new Date(Date.now() + 288e5);
            const p = (n, l = 2) => String(n).padStart(l, '0');
            return `${d.getUTCFullYear()}-${p(d.getUTCMonth() + 1)}-${p(d.getUTCDate())} ${p(d.getUTCHours())}:${p(d.getUTCMinutes())}:${p(d.getUTCSeconds())},${p(d.getUTCMilliseconds(), 3)}`;
        },
        _dyid_link: (text) => String(text).replace(/\b(\d{15,19})\b/g, (_, id) =>
            `\x1b]8;;https://t.bilibili.com/${id}\x07${id}\x1b]8;;\x07`
        ),
        _cache: [],
        /**
         * 初始化默认level为3
        ...
        debug(context, msg) {
            if (this._level >= 4) {
                ...
                const time = this._iso_time();
                const name = process.env['NOTE'] || `帐号${process.env['NUMBER']}`;
                this.proPrint([
                    this._colors[0](`[${time}]`),
                    this._colors[1]('[DEBUG]:'),
                    this._colors[2](`${name}:`),
                    this._colors[3](this._dyid_link(`${context} ${msg}`)),
                ].join(' '));
            }
        },
        info(context, msg) {
            if (this._level >= 3) {
                ...
                this._cache.push(`[${time}] [INFO]: ${name}: ${text}`);
                this.proPrint([
                    this._colors[0](`[${time}]`),
                    this._colors[1]('[INFO]:'),
                    this._colors[2](`${name}:`),
                    this._colors[4](this._dyid_link(text)),
                ].join(' '));
            }
        },
        notice(context, msg) {
            if (this._level >= 2) {
                ...
                const name = process.env['NOTE'] || `帐号${process.env['NUMBER']}`;
                const text = `${context} ${msg}`;
                this._cache.push(`[${time}] [NOTICE]: ${name}: ${text}`);
                this.proPrint([
                    this._colors[0](`[${time}]`),
                    this._colors[1]('[NOTICE]:'),
                    this._colors[2](`${name}:`),
                    this._colors[5](this._dyid_link(text)),
                ].join(' '));
            }
        },
        warn(context, msg) {
            if (this._level >= 1) {
                ...
                const name = process.env['NOTE'] || `帐号${process.env['NUMBER']}`;
                const text = `${context} ${msg}`;
                this._cache.push(`[${time}] [WARN]: ${name}: ${text}`);
                this.proPrint([
                    this._colors[0](`[${time}]`),
                    this._colors[1]('[WARN]:'),
                    this._colors[2](`${name}:`),
                    this._colors[6](this._dyid_link(text)),
                ].join(' '));
            }
        },
        error(context, msg) {
            if (this._level >= 0) {
                ...
                const name = process.env['NOTE'] || `帐号${process.env['NUMBER']}`;
                const text = `${context} ${msg}`;
                this._cache.push(`[${time}] [ERROR]: ${name}: ${text}`);
                this.proPrint([
                    this._colors[0](`[${time}]`),
                    this._colors[1]('[ERROR]:'),
                    this._colors[2](`${name}:`),
                    this._colors[7](this._dyid_link(text)),
                ].join(' '));
            }
        },
        flushToFile() {
            ...
            fs.appendFileSync(log_file, this._cache.join('\n') + '\n', 'utf8');
